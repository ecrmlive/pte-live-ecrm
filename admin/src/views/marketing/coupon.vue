<template>
  <a-card :bordered="false" class="page-card">
    <div class="toolbar">
      <a-button v-if="canCreate" type="primary" @click="openCreate">新建平台券</a-button>
    ...
    <a-modal v-model:open="createOpen" title="新建平台券" :confirm-loading="saving" @ok="submitCreate">
...
async function submitCreate() {
  saving.value = true;
  try {
    const limited = form.total_count > 0 ? 1 : 0;
    await createCoupon({
      title: form.title,
...
async function toggle(row: Coupon) {
  ...
  try {
    await setCouponStatus(row.coupon_id, next);
    message.success('已更新');
    load();
  } catch (e) {
