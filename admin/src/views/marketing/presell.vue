<template>
  <a-card :bordered="false" class="page-card">
    <div class="toolbar">
      ...
      <span class="hint">全款预售监管 · 定金模式顺延</span>
    </div>
    <a-table
...
import { deletePresell, fetchPresells, updatePresell, type PresellActive } from '@/api/presell';
...
async function toggle(row: PresellActive) {
  const next = row.is_show === 1 ? 0 : 1;
  await updatePresell(row.product_presell_id, { is_show: next, price: row.price });
