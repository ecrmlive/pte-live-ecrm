...
export interface SpreadLog {
  user_spread_log_id: number;
  uid: number;
  old_spread_uid: number;
  spread_uid: number;
  create_time?: string;
}
...
export function fetchSpreadLogs(params: Record<string, unknown>) {
  return http.get<PageResult<SpreadLog>>('/spread/logs', { params });
}

export function fetchSpreadBills(params: Record<string, unknown>) {
  return http.get<PageResult<Record<string, unknown>>>('/spread/bills', { params });
}
