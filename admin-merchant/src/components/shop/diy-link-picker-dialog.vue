...
const props = defineProps<{ is_linkset?: boolean; linkData?: ShopLinkValue | null }>();
const emit = defineEmits<{ closeDialog: [payload?: ShopLinkValue] }>();
...
async function load() {
  loading.value = true;
  try {
    const [categoryRes, linkRes, microRes] = await Promise.all([listDiyPageCategoriesApi(), listDiyPageLinksApi({ page: 1, limit: 100 }), listDiyPagesApi({ is_diy: 0, status: 1, page: 1, limit: 100 })]);
...
<template>
  <ElDialog v-model="open" title="选择商户页面链接" width="720px" destroy-on-close @closed="close">
    <ElRadioGroup v-model="source" class="mb-3"><ElRadioButton value="link">系统链接</ElRadioButton><ElRadioButton value="micro">本店微页面</ElRadioButton></ElRadioGroup>
