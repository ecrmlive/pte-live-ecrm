...
async function upload({ file }: { file: File }) {
  if (props.kind === 'image' ? !file.type.startsWith('image/') : !file.type.startsWith('video/')) {
    ElMessage.warning(props.kind === 'image' ? '请选择图片文件' : '请选择视频文件');
    return;
  }
