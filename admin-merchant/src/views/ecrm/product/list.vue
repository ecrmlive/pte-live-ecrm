...
async function remove(row: MerchantProduct) {
  try {
    await ElMessageBox.confirm(`删除商品“${row.store_name}”后不可恢复，是否继续？`, '删除确认', { type: 'warning' });
...
<template>
  <Page title="商品列表" description="管理本店商品、库存与上架状态；商品编辑会按店铺审核规则重新进入审核流程。">
    <template #extra><el-button type="primary" @click="openCreate">新增商品</el-button></template>
