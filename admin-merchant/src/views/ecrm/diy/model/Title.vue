<template>
	<div>
		<div @click.stop="diyEditer(index)">
			<div
				class="drag optional"
				:style="{
			...
			>
				<div
					class="diy-title"
					:key="'title-style-' + titleType"
					:class="'diy-title-' + titleType"
					:style="{
				...
				>
					<!-- 风格五的副标题 -->
					<div v-if="titleType == 5" :style="{ color: item.style.subtextColor, fontSize: item.style.subtextSize + 'px' }">{{ item.params.subtitle }}</div>
					<!-- 风格八的 -->
					<div
						v-if="titleType == 8 && item.style.isLine"
						class="type8-before"
						:style="{ background: item.style.lineColor || '', height: item.style.textSize + 'px' }"
					></div>
					<div class="d-b-c">
						<!-- 风格六的副标题 -->
						<span class="text-type6" v-if="titleType == 6" :style="{ color: item.style.subtextColor, fontSize: item.style.subtextSize + 'px' }">
							{{ item.params.subtitle }}
						</span>
						<!-- 风格四的图形 -->
						<div class="text-type4" v-if="titleType == 4">
							<div class="line" :style="{ background: item.style.textColor }"></div>
							<div class="dot" :style="{ borderColor: item.style.textColor }"></div>
						</div>
						<!-- 风格一的图形 -->
						<div class="text-type1" style="margin-right: 20px;" v-if="titleType == 1">
						...
						<!-- 风格一的图形 -->
						<div class="text-type1" style="margin-left: 20px;" v-if="titleType == 1">
							<span class="line fb" :style="{ color: item.style.lineColor || '' }"></span>
							<span class="line fb op3" :style="{ color: item.style.lineColor || '' }"></span>
						</div>
						<!-- 风格三的底部线条 -->
						<div class="title-line" :style="{ background: item.style.lineColor || '' }" v-if="titleType == 2"></div>
						<!-- 风格三的底部方块 -->
						<div class="title-line3" v-if="titleType == 3"><div class="block3" :style="{ background: themeColor }"></div></div>
					</div>
					<!-- 风格八的副标题 -->
					<div class="flex-1 d-s-c" v-if="titleType == 8">
						<div
							class="type8-subbox"
							v-if="titleType == 8 && item.style.isSub"
					...
					<!-- 风格八的更多 -->
					<div v-if="titleType == 8 && item.style.isMore" class="more" :style="{ color: item.style.moretextColor }">{{ item.params.moretitle }}</div>
					<!-- 风格七的副标题 -->
					<div
						class="type7-subbox"
						v-if="titleType == 7"
					...
					<!-- 风格五的底部方块 -->
					<div class="block5" v-if="titleType == 5" :style="{ background: themeColor }"></div>
					<!-- 风格六的底部方块 -->
					<div class="block6" v-if="titleType == 6" :style="{ background: themeColor }"></div>
					<!-- 风格四的底部文字 -->
					<div v-if="titleType == 4" :style="{ color: item.style.subtextColor, fontSize: item.style.subtextSize + 'px' }">{{ item.params.subtitle }}</div>
				</div>
...
export default {
  ...
  inject: ['diyModel'],
	...
	computed: {
		titleType() {
			return Number(this.item.style?.type ?? 1);
		},
	},
	methods: {
  diyEditer(index) {
