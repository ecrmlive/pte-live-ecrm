...
async function load() {
  loading.value = true;
  try {
    const [page, currentBalance] = await Promise.all([
      listMerchantWithdrawsApi(query),
      getMerchantBalanceApi(),
    ]);
...
<template>
  <EcrmListPage
    title="提现管理"
    description="仅可提交提现申请和查看本商户记录；审核、拒绝、打款及凭证均由平台后台处理。"
  >
    <template #filters>
    ...
    <el-table v-loading="loading" :data="rows" border row-key="financial_id" stripe>
      <el-table-column label="申请单号" min-width="180" prop="financial_sn" />
      <el-table-column label="提现金额" width="116">
        <template #default="{ row }">¥{{ Number(row.extract_money).toFixed(2) }}</template>
      </el-table-column>
      <el-table-column label="收款方式" width="104">
        <template #default="{ row }">{{ accountType(row.financial_type) }}</template>
      </el-table-column>
      <el-table-column
        label="收款账户"
        min-width="150"
        prop="financial_account"
        show-overflow-tooltip
      />
      <el-table-column label="审核状态" width="116">
        <template #default="{ row }">
          <el-tag :type="auditInfo(row.status).type" size="small">
      ...
      <el-table-column label="打款状态" width="100">
        <template #default="{ row }">
          <el-tag :type="transferInfo(row).type" size="small">
    ...
    <el-dialog v-model="applyOpen" destroy-on-close title="申请提现" width="500px">
      <el-form label-width="96px">
        <el-form-item label="可用余额">
          <span>¥{{ Number(balance).toFixed(2) }}</span>
        </el-form-item>
        <el-form-item label="提现金额" required>
          <el-input-number v-model="form.extract_money" :min="0.01" :precision="2" class="w-full" />
        </el-form-item>
        <el-form-item label="收款方式" required>
          <el-select v-model="form.financial_type" class="w-full">
        ...
        <el-form-item label="收款账户" required>
          <el-input v-model="form.financial_account" placeholder="银行卡号、微信号或支付宝账号" />
        </el-form-item>
      ...
      <template #footer>
        ...
        <el-button :loading="saving" type="primary" @click="apply">提交申请</el-button>
      </template>
    </el-dialog>

    <el-drawer v-model="detailOpen" :with-header="false" size="560px">
      <template v-if="detail">
        <div class="mb-5 text-lg font-medium">提现详情</div>
        <el-descriptions :column="1" border>
          <el-descriptions-item label="申请单号">{{ detail.financial_sn }}</el-descriptions-item>
          ...
          <el-descriptions-item label="收款账户">{{ detail.financial_account }}</el-descriptions-item>
          <el-descriptions-item label="审核状态">
            <el-tag :type="auditInfo(detail.status).type">{{ auditInfo(detail.status).label }}</el-tag>
          </el-descriptions-item>
          <el-descriptions-item label="打款状态">
            <el-tag :type="transferInfo(detail).type">{{ transferInfo(detail).label }}</el-tag>
          </el-descriptions-item>
          <el-descriptions-item v-if="detail.refusal" label="拒绝原因">
            {{ detail.refusal }}
          </el-descriptions-item>
          <el-descriptions-item label="申请备注">{{ detail.mark || '—' }}</el-descriptions-item>
