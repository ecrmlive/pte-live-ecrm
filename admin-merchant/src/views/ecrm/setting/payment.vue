...
import { ElAlert, ElButton, ElCard, ElDialog, ElForm, ElFormItem, ElInput, ElMessage, ElSkeleton, ElSwitch, ElTag } from 'element-plus';
...
async function save() {
  ...
  saving.value = true;
  try { await updateMerchantPaymentChannelApi(editing.value, { ...form }); await load(); dialogOpen.value = false; ElMessage.success('本店支付配置已加密保存'); } finally { saving.value = false; }
}
onMounted(load);
</script>

<template>
  <Page title="店铺支付配置" description="每个店铺独立维护自己的微信支付与支付宝商户账号；绝不复用平台支付账户。">
