...
import AccessFormModal from '../access/access-form-modal.vue';
import type { AccessAddType, AccessNode } from '../access/types';
...
async function loadTableList() {
  loading.value = true;
  try {
    const res = await AccessApi.accessList({}, true);
...
async function handleDelete(row: AccessNode) {
  ...
  loading.value = true;
  try {
    const res = await AccessApi.delAccess({ access_id: row.access_id }, true);
    if (res.code === 1) {
...
<template>
  <Page v-loading="loading">
    <div class="mb-3 flex w-full flex-wrap items-center justify-between gap-3">
        <ElButton
          v-access:code="'platform:access:add'"
          :icon="Plus"
          type="primary"
          @click="openAdd()"
        >
    ...
    >
      ...
      <ElTableColumn fixed="right" label="操作" width="280">
        <template #default="{ row }">
          ...
          <ElButton
            v-access:code="'platform:access:edit'"
            link
            type="primary"
            @click="openEdit(row)"
          >
          ...
          <ElButton
            v-access:code="'platform:access:delete'"
            link
            type="primary"
