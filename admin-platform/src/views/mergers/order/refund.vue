...
function canAudit(row: PlatformRefundOrder) {
  return isPlatformOperator.value && (row.status === 0 || row.status === 4) && row.refund_type === 1;
}
...
<template>
  <Page title="退款监管" description="平台账号可处理待审核或平台介入的仅退款；区域账号只能查看所属区域商户的退款数据。退货退款只展示，不在此页改变状态。">
    ...
    <el-card class="mt-4" shadow="never">
      <el-table v-loading="loading" :data="rows" row-key="refund_order_id">
        ...
        <el-table-column label="状态" width="110"><template #default="{ row }"><el-tag :type="statusInfo(row.status).type">{{ statusInfo(row.status).label }}</el-tag></template></el-table-column>
