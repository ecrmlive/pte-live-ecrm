...
<template>
  <Page title="付费会员" description="管理 C 端用户的 SVIP 状态；会员折扣与优惠券叠加规则由订单计价服务统一执行。">
    ...
    <el-dialog v-model="dialogOpen" title="设置付费会员" width="480px" destroy-on-close><el-form label-width="96px"><el-form-item label="用户"><span>{{ editing?.nickname || editing?.account }}</span></el-form-item><el-form-item label="会员类型"><el-select v-model="form.is_svip" class="w-full"><el-option label="普通用户" :value="0" /><el-option label="体验会员" :value="1" /><el-option label="有效期会员" :value="2" /><el-option label="永久会员" :value="3" /><el-option label="关闭会员" :value="-1" /></el-select></el-form-item><el-form-item v-if="form.is_svip === 2" label="到期时间" required><el-date-picker v-model="form.svip_endtime" class="w-full" format="YYYY-MM-DD HH:mm:ss" value-format="YYYY-MM-DD HH:mm:ss" type="datetime" /></el-form-item></el-form><template #footer><el-button @click="dialogOpen = false">取消</el-button><el-button :loading="saving" type="primary" @click="save">保存</el-button></template></el-dialog>
  </Page>
</template>
