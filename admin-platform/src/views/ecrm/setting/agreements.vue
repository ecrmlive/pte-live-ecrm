...
const loading = ref(false); const saving = ref(false); const rows = ref<PlatformAgreement[]>([]); const current = ref<PlatformAgreement>(); const content = ref(''); const canManage = ref(false);
...
onMounted(async () => { const [permissions] = await Promise.all([getAccessCodesApi(), load()]); canManage.value = permissions.includes('setting.agreement.manage'); });
</script>
<template>
  <Page title="协议规则" description="维护 C 端协议正文；保存操作受统一后台按钮权限控制。"><div class="grid gap-4 lg:grid-cols-[280px_1fr]"><el-card v-loading="loading" shadow="never"><template #header>协议列表</template><el-menu :default-active="current?.key" @select="(key) => { const row = rows.find((item) => item.key === key); if (row) select(row); }"><el-menu-item v-for="row in rows" :key="row.key" :index="row.key">{{ row.label }}</el-menu-item></el-menu></el-card><el-card shadow="never"><template #header>{{ current?.label || '请选择协议' }}</template><template v-if="current"><el-input v-model="content" :disabled="!canManage" :rows="18" type="textarea" placeholder="请输入协议正文" /><div class="mt-4 flex justify-end"><el-button v-if="canManage" :loading="saving" type="primary" @click="save">保存协议</el-button></div></template></el-card></div></Page>
</template>
