...
<template>
  <Page
    title="店铺结算监管"
    description="平台可读取全量投影，区域角色仅可读取授权区域。平台审核与内部凭证登记仅发送受控命令；不直连商户库、不发起转账，也不录入收款账户或密钥。"
  >
    <el-alert
      v-if="!canRead"
      class="mb-4"
    ...
    <template v-else>
      <el-row :gutter="16" class="mb-4">
        <el-col
          v-for="status in (['bill_frozen', 'withdraw_applied', 'paid'] as MerchantSettlementStatus[])"
          :key="status"
          :md="8"
          :xs="24"
        >
      ...
      <EcrmListPage title="结算列表">
        ...
        <el-table v-loading="loading" :data="rows" row-key="settlement_id">
          ...
          <el-table-column label="结算周期" min-width="300">
          ...
          <el-table-column label="投影更新时间" prop="updated_at" min-width="180" />
          <el-table-column v-if="canReview" fixed="right" label="操作" width="170">
