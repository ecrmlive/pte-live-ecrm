...
async function exportRows() {
  try {
    ...
    ElMessage.success(`已导出 ${result.row_count} 条退款监管记录${result.truncated ? '（已按 5000 条上限截断）' : ''}`);
  } catch {
    // 用户取消或接口错误时由统一请求层提示。
  }
}
...
<template>
  <Page title="退款监管" description="平台账号可处理待审核或平台介入的售后；区域账号仅查看所属区域商户数据。仅退款进入支付渠道退款阶段；退货退款需等待用户寄回与商户确认收货。">
    ...
    <el-card class="mt-4" shadow="never">
      <el-table v-loading="loading" :data="rows" row-key="refund_order_id">
        ...
        <el-table-column label="售后类型" width="104"><template #default="{ row }">{{ refundType(row.refund_type) }}</template></el-table-column>
        <el-table-column label="退款金额" width="116"><template #default="{ row }">¥{{ Number(row.refund_price).toFixed(2) }}</template></el-table-column>
        <el-table-column label="状态" width="110"><template #default="{ row }"><el-tag :type="statusInfo(row.status_code).type">{{ statusInfo(row.status_code).label }}</el-tag></template></el-table-column>
      ...
      <div class="mt-4 flex justify-end">
        <el-pagination :current-page="query.page" :page-size="query.limit" :page-sizes="[10, 20, 50, 100]" :total="total" background layout="total, sizes, prev, pager, next" @current-change="(page: number) => { query.page = page; load(); }" @size-change="(limit: number) => { query.limit = limit; query.page = 1; load(); }" />
      </div>
    </el-card>
    ...
    <el-drawer v-model="detailOpen" :with-header="false" size="640px">
      <template v-if="current">
        ...
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="状态"><el-tag :type="statusInfo(current.status_code).type">{{ statusInfo(current.status_code).label }}</el-tag></el-descriptions-item>
