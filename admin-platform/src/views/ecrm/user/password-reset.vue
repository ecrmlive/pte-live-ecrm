...
<template><Page title="用户密码重置" description="仅平台角色可重置本地 PC 登录密码。新密码只用于 bcrypt 哈希，旧令牌会因身份版本递增立即失效。"><el-alert class="mb-4" type="warning" :closable="false" title="不显示、记录或回传密码；请仅使用受控临时密码，并以安全渠道交付给用户。"/><el-card shadow="never" class="max-w-3xl"><el-form label-width="116px" @submit.prevent="submit"><el-form-item label="关联用户" required><UserRelationSelect v-model="form.user_id" /></el-form-item><el-form-item label="新密码" required><el-input v-model="form.password" type="password" show-password autocomplete="new-password"/></el-form-item><el-form-item label="确认新密码" required><el-input v-model="form.confirm_password" type="password" show-password autocomplete="new-password"/></el-form-item><el-form-item label="重置原因" required><el-input v-model="form.reason" type="textarea" :rows="4" maxlength="500" show-word-limit/></el-form-item><el-form-item><el-button type="primary" :loading="submitting" @click="submit">确认重置</el-button></el-form-item></el-form></el-card></Page></template>
