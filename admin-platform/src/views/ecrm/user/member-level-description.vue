...
/** 对齐 CRMEB CacheRepository::SYS_MEMBER */
const AGREE_KEY = 'sys_member';
const PAGE_TITLE = '等级规则说明';
...
async function save() {
  ...
  if (!html || html === '<p></p>') {
    ElMessage.warning('等级规则说明正文不能为空');
    return;
  }
  saving.value = true;
  try {
...
onMounted(async () => {
  ...
  canManage.value =
    codes.includes('user.level.description.manage') ||
...
<template>
  <Page auto-content-height content-class="!p-0">
    <SettingsTabLayout v-loading="loading">
      <div class="member-level-description">
        <div class="member-level-description__title">{{ PAGE_TITLE }}</div>
        <VbenTiptap
          v-model="content"
          :editable="canManage"
...
<style scoped>
.member-level-description__title {
  margin-bottom: 16px;
  font-size: 16px;
  font-weight: 600;
  line-height: 24px;
