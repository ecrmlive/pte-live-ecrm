...
<template><Page title="用户启停" description="仅平台角色可执行。停用或重新启用都会递增身份版本，使既有 C 端令牌失效；不会取消订单、退款、资产、优惠券或佣金事实。"><el-alert class="mb-4" type="warning" :closable="false" title="停用只阻断新的受保护 C 端访问，不自动撤销已产生的交易或资金记录。"/><el-card shadow="never" class="max-w-3xl"><el-form label-width="116px" @submit.prevent="submit"><el-form-item label="用户 ID" required><el-input-number v-model="form.user_id" :min="1" controls-position="right"/></el-form-item><el-form-item label="目标状态" required><el-radio-group v-model="form.status"><el-radio :value="0">停用</el-radio><el-radio :value="1">启用</el-radio></el-radio-group></el-form-item><el-form-item label="调整原因" required><el-input v-model="form.reason" type="textarea" :rows="4" maxlength="500" show-word-limit placeholder="例如：虚构中文风控工单演示，暂时停用账号。"/></el-form-item><el-form-item><el-button type="primary" :loading="submitting" @click="submit">确认调整</el-button></el-form-item></el-form></el-card></Page></template>
