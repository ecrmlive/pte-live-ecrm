...
const form = reactive({ user_ids_text: '', label_ids: [] as number[], reason: '' });
...
function reset() { Object.assign(form, { user_ids_text: '', label_ids: [], reason: '' }); }
async function submit() {
  const userIDs = parseUserIDs();
  const reason = form.reason.trim();
  if (!userIDs || form.label_ids.length > 50 || reason.length < 2 || reason.length > 500) { ElMessage.warning('请填写 1 至 100 个用户 ID、至多 50 个标签及调整原因'); return; }
