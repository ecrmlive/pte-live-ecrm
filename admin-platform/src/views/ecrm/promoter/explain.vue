...
/** 对齐 CRMEB CacheRepository::PROMOTER_EXPLAIN */
const AGREE_KEY = 'promoter_explain';
const PAGE_TITLE = '分销说明';
...
onMounted(async () => {
  ...
  canManage.value =
    ...
    codes.includes('promoter.explain');
  await load();
});
</script>
...
<template>
  <Page auto-content-height content-class="!p-0">
    <SettingsTabLayout v-loading="loading">
      <div class="promoter-explain">
        <div class="promoter-explain__title">{{ PAGE_TITLE }}</div>
        <VbenTiptap
          v-model="content"
          :editable="canManage"
