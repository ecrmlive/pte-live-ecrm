...
/** 对齐 CRMEB CacheRepository::SYS_BROKERAGE */
const AGREE_KEY = 'sys_brokerage';
const PAGE_TITLE = '分销等级规则';
...
onMounted(async () => {
  ...
  canManage.value =
    ...
    codes.includes('promoter.brokerage.rule');
  await load();
});
</script>
...
<template>
  <Page auto-content-height content-class="!p-0">
    <SettingsTabLayout v-loading="loading">
      <div class="distribution-rule">
        <div class="distribution-rule__title">{{ PAGE_TITLE }}</div>
        <VbenTiptap
          v-model="content"
          :editable="canManage"
