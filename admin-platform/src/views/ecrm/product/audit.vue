...
async function approve(row: PlatformProduct) {
  try {
    await ElMessageBox.confirm(`确认审核通过商品“${row.title}”？`, '商品审核', { type: 'warning' });
...
onMounted(async () => {
  ...
  canAudit.value = profile.roles.includes('platform') && permissions.includes('product.audit.submit');
  await load();
});
</script>

<template>
  <Page title="商品监管" description="平台账号可按按钮权限审核商品；商户和区域账号仅能查看服务端授权范围内的商品。">
