...
/** 对齐 CRMEB CacheRepository::PRESELL_AGREE */
const AGREE_KEY = 'sys_product_presell_agree';
const PAGE_TITLE = '预售协议';
...
onMounted(async () => {
  ...
  canManage.value =
    ...
    codes.includes('marketing.presell.agreement');
  await load();
});
</script>
...
<template>
  <Page auto-content-height content-class="!p-0">
    <SettingsTabLayout v-loading="loading">
      <div class="presell-agreement">
        <div class="presell-agreement__title">{{ PAGE_TITLE }}</div>
        <VbenTiptap
          v-model="content"
          :editable="canManage"
