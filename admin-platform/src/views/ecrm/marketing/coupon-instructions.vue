...
/** 对齐 CRMEB CacheRepository::COUPON_AGREE */
const AGREE_KEY = 'sys_coupon_agree';
const PAGE_TITLE = '使用说明';
...
onMounted(async () => {
  ...
  canManage.value =
    ...
    codes.includes('marketing.platform_coupon.help');
  await load();
});
</script>
  ...
  <Page auto-content-height content-class="!p-0">
    <SettingsTabLayout v-loading="loading">
      <div class="coupon-instructions">
        <div class="coupon-instructions__title">{{ PAGE_TITLE }}</div>
        <VbenTiptap
          v-model="content"
          :editable="canManage"
