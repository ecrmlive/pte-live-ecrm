...
import {
  fetchMarketingDiscounts,
  saveMarketingDiscounts,
} from '#/api/core/platform-marketing-cache';
...
<template>
  <Page title="满减活动" description="基于 setting_cache 的 stub 配置；真实营销规则待业务域 API 接入后替换。">
    <template #extra><el-button type="primary" @click="open()">新增活动</el-button></template>
    ...
    <el-dialog v-model="dialog" :title="`${editingIndex === undefined ? '新增' : '编辑'}满减活动`" width="520px" destroy-on-close>
