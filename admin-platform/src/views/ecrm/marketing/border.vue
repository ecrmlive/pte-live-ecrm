...
/** 示例预览：商品图 + 角上边框效果（可访问占位图） */
const EXAMPLE_PRODUCT =
  'https://picsum.photos/seed/qixi-border-product/240/240';
const EXAMPLE_FRAME =
  'https://picsum.photos/seed/qixi-border-frame/750/750';
...
function openEdit(row: MarketingDecor) {
  ...
  const scopeType = Number(
    row.scope_type ?? row.payload?.scope_type ?? 0,
  ) as MarketingDecorScopeType;
  Object.assign(form, {
    name: row.name || '',
    ...
    scope_type: ([0, 1, 2, 3, 4].includes(scopeType)
      ? scopeType
      : 0) as MarketingDecorScopeType,
  ...
  formDrawerApi.setState({ title: '编辑活动边框图' }).open();
}
...
async function save() {
  ...
  try {
    if (editing.value) {
      await updateMarketingDecorApi('border', editing.value.id, body);
    } else {
      await createMarketingDecorApi('border', body);
    }
    formDrawerApi.close();
...
async function changeStatus(row: MarketingDecor, enabled: boolean) {
  ...
  try {
    await setMarketingDecorStatusApi('border', row.id, enabled ? 1 : 0);
  } catch {
    row.status = before;
  }
}
...
async function removeRow(row: MarketingDecor) {
  try {
    await confirm({
      content: `确定删除活动边框「${row.name}」吗？`,
      icon: 'warning',
      title: '提示',
    });
    await deleteMarketingDecorApi('border', row.id);
...
onMounted(async () => {
  const [profile, codes, cateRes, labelRes] = await Promise.all([
    getUserInfoApi(),
    getAccessCodesApi(),
    listPlatformCategoriesApi().catch(
      () => ({ list: [] as PlatformCategory[] }),
    ),
  ...
  labelOptions.value = (labelRes.list || []).filter(
    (x) => Number(x.status) !== 0,
  );
  ...
  canRead.value =
    roleOK &&
    (codes.includes('marketing.border.read') ||
      codes.includes('marketing.border.manage'));
  canManage.value = roleOK && codes.includes('marketing.border.manage');
    ...
    <FormDrawer>
      <ElTabs v-model="drawerTab">
        <ElTabPane label="基础设置" name="basic">
          <ElForm label-width="110px" class="border-form">
            <ElFormItem label="边框名称" required>
              <ElInput
                v-model="form.name"
                maxlength="128"
                show-word-limit
    ...
    <ExampleModal>
      <div class="example-phone">
        ...
        <div class="example-phone__body">
          <div class="example-phone__title">商品列表</div>
          <div class="example-grid">
            <div v-for="i in 4" :key="i" class="example-card">
              <div class="example-card__media">
                <img :src="EXAMPLE_PRODUCT" alt="" class="example-card__img" />
                <img
                  :src="EXAMPLE_FRAME"
                  alt=""
                  class="example-card__frame"
                />
              </div>
              <div class="example-card__name">演示商品 {{ i }}</div>
              <div class="example-card__price">¥99.00</div>
            </div>
          </div>
        </div>
        <div class="field-hint example-hint">建议尺寸：宽 750px × 高 750px</div>
      </div>
    </ExampleModal>
