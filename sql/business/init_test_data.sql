...
-- 数据大屏专用本地演示数据：全部为明确标识的虚构中文记录。
-- 固定 ID 只更新本组数据，可重复导入；封面位于 admin-platform/public/demo/data-screen，
-- 真实 COS 配置启用后由附件中心上传并替换这些本地地址。
INSERT INTO `qixi_crm_b_store_view` (`store_id`,`merchant_id`,`store_app_id`,`store_name`,`status`,`integral_enabled`,`integral_points_per_yuan`,`integral_max_deduction_bps`) VALUES
...
INSERT INTO `qixi_crm_b_product_view` (`product_id`,`merchant_id`,`store_id`,`merchant_name`,`store_name`,`category_id`,`title`,`cover_url`,`price`,`original_price`,`product_type`,`sales`,`stock`,`sale_status`,`version`,`updated_at`) VALUES
  (1501,11,11,'云岚风物商户','云岚风物旗舰店',10601,'云海雪山摄影艺术画', '/demo/data-screen/bg_1.png',698.00,898.00,0,428,80,1,1,NOW()),
  (1502,12,12,'星野旅行商户','星野旅行生活馆',10601,'湖畔晨光旅行画册', '/demo/data-screen/bg_9.png',488.00,628.00,0,396,96,1,1,NOW()),
  (1503,13,13,'山海影像商户','山海影像精选店',10601,'旷野日落高清装饰画', '/demo/data-screen/bg_10.png',788.00,998.00,0,371,72,1,1,NOW()),
  (1504,14,14,'林泉露营商户','林泉露营装备店',10601,'森林秘境露营礼盒', '/demo/data-screen/bg_11.png',968.00,1288.00,0,318,58,1,1,NOW()),
  (1505,15,15,'晴川艺术商户','晴川艺术画廊',10601,'霞光山脉收藏级画芯', '/demo/data-screen/bg_12.jpg',1288.00,1588.00,0,286,48,1,1,NOW()),
  (1506,16,16,'远方自然商户','远方自然美学店',10601,'碧海晴空艺术微喷', '/demo/data-screen/bg_13.jpg',1088.00,1388.00,0,265,45,1,1,NOW()),
  (1507,17,17,'云端摄影商户','云端摄影器材店',10601,'山谷云层风景摄影集', '/demo/data-screen/bg_14.jpg',898.00,1188.00,0,243,61,1,1,NOW()),
  (1508,18,18,'晨雾家居商户','晨雾家居优选店',10601,'自然风光沉浸式画框', '/demo/data-screen/bg_11.png',568.00,728.00,0,221,88,1,1,NOW())
...
INSERT INTO `qixi_crm_b_user` (`id`,`nickname`,`mobile`,`status`,`group_id`,`auth_version`,`created_at`) WITH RECURSIVE `screen_seq` AS (
  SELECT 1 AS `n` UNION ALL SELECT `n` + 1 FROM `screen_seq` WHERE `n` < 24
)
SELECT 97200 + `n`, CONCAT('大屏演示用户', LPAD(`n`, 2, '0')), CONCAT('SCREEN-DEMO-', LPAD(`n`, 4, '0')), 1, 0, 1,
  CASE WHEN `n` <= 12 THEN NOW() ELSE DATE_SUB(NOW(), INTERVAL 15 DAY) END
FROM `screen_seq`
...
INSERT INTO `qixi_crm_b_group_order` (`id`,`order_no`,`user_id`,`total_amount`,`discount_amount`,`freight_amount`,`pay_amount`,`total_quantity`,`activity_type`,`points_amount`,`recipient_snapshot`,`pay_channel`,`pay_status`,`paid_at`,`idempotency_key`,`remark`) WITH RECURSIVE `screen_seq` AS (
  SELECT 1 AS `n` UNION ALL SELECT `n` + 1 FROM `screen_seq` WHERE `n` < 72
)
SELECT 975000 + `n`, CONCAT('SCREEN-DEMO-G-', DATE_FORMAT(CURDATE(), '%Y%m%d'), '-', LPAD(`n`, 3, '0')), 97200 + MOD(`n` - 1, 24) + 1,
  588.00 + MOD(`n`, 8) * 137.00, 0.00, 0.00, 588.00 + MOD(`n`, 8) * 137.00, 1 + MOD(`n`, 3), 0, 0,
  JSON_OBJECT('recipient','大屏演示收件人','province',ELT(MOD(`n` - 1, 12) + 1,'北京市','上海市','广东省','浙江省','江苏省','四川省','湖北省','山东省','福建省','陕西省','辽宁省','云南省')),
  ELT(MOD(`n` - 1, 3) + 1, 'wechat', 'alipay', 'balance'), 'paid', CASE WHEN `n` <= 24 THEN DATE_SUB(NOW(), INTERVAL (`n` * 47) MINUTE) ELSE DATE_SUB(NOW(), INTERVAL (`n` - 24) DAY) END,
  CONCAT('screen-demo-group-', LPAD(`n`, 3, '0')), '数据大屏本地演示订单'
FROM `screen_seq`
ON DUPLICATE KEY UPDATE `user_id`=VALUES(`user_id`),`total_amount`=VALUES(`total_amount`),`pay_amount`=VALUES(`pay_amount`),`total_quantity`=VALUES(`total_quantity`),`recipient_snapshot`=VALUES(`recipient_snapshot`),`pay_channel`=VALUES(`pay_channel`),`pay_status`='paid',`paid_at`=VALUES(`paid_at`),`remark`=VALUES(`remark`),`updated_at`=NOW();
...
SELECT 975000 + `n`, 975000 + `n`, CONCAT('SCREEN-DEMO-O-', DATE_FORMAT(CURDATE(), '%Y%m%d'), '-', LPAD(`n`, 3, '0')), 10 + MOD(`n` - 1, 8) + 1,
  ELT(MOD(`n` - 1, 8) + 1,'云岚风物商户','星野旅行商户','山海影像商户','林泉露营商户','晴川艺术商户','远方自然商户','云端摄影商户','晨雾家居商户'),
  10 + MOD(`n` - 1, 8) + 1,
  ELT(MOD(`n` - 1, 8) + 1,'云岚风物旗舰店','星野旅行生活馆','山海影像精选店','林泉露营装备店','晴川艺术画廊','远方自然美学店','云端摄影器材店','晨雾家居优选店'),
  97200 + MOD(`n` - 1, 24) + 1, 588.00 + MOD(`n`, 8) * 137.00, 0.00, 0.00, 588.00 + MOD(`n`, 8) * 137.00, 1 + MOD(`n`, 3), 0, 0,
  JSON_OBJECT('recipient','大屏演示收件人','province',ELT(MOD(`n` - 1, 12) + 1,'北京市','上海市','广东省','浙江省','江苏省','四川省','湖北省','山东省','福建省','陕西省','辽宁省','云南省')),
  '数据大屏本地演示订单', ELT(MOD(`n` - 1, 4) + 1,'paid','fulfilling','shipped','completed'),
  CASE WHEN `n` <= 24 THEN DATE_SUB(NOW(), INTERVAL (`n` * 47) MINUTE) ELSE DATE_SUB(NOW(), INTERVAL (`n` - 24) DAY) END
FROM `screen_seq`
ON DUPLICATE KEY UPDATE `merchant_id`=VALUES(`merchant_id`),`merchant_name_snapshot`=VALUES(`merchant_name_snapshot`),`store_id`=VALUES(`store_id`),`store_name_snapshot`=VALUES(`store_name_snapshot`),`user_id`=VALUES(`user_id`),`total_amount`=VALUES(`total_amount`),`pay_amount`=VALUES(`pay_amount`),`total_quantity`=VALUES(`total_quantity`),`recipient_snapshot`=VALUES(`recipient_snapshot`),`status`=VALUES(`status`),`paid_at`=VALUES(`paid_at`),`updated_at`=NOW();
...
SELECT 976000 + `n`, 975000 + `n`, 1500 + MOD(`n` - 1, 8) + 1, 0, CONCAT('screen-', MOD(`n` - 1, 8) + 1),
  ELT(MOD(`n` - 1, 8) + 1,'云海雪山摄影艺术画','湖畔晨光旅行画册','旷野日落高清装饰画','森林秘境露营礼盒','霞光山脉收藏级画芯','碧海晴空艺术微喷','山谷云层风景摄影集','自然风光沉浸式画框'),
  ELT(MOD(`n` - 1, 7) + 1,'/demo/data-screen/bg_1.png','/demo/data-screen/bg_9.png','/demo/data-screen/bg_10.png','/demo/data-screen/bg_11.png','/demo/data-screen/bg_12.jpg','/demo/data-screen/bg_13.jpg','/demo/data-screen/bg_14.jpg'),
  JSON_OBJECT('规格','大屏演示款'), 588.00 + MOD(`n`, 8) * 137.00, 1 + MOD(`n`, 3), 0
FROM `screen_seq`
ON DUPLICATE KEY UPDATE `product_id`=VALUES(`product_id`),`sku_key`=VALUES(`sku_key`),`title_snapshot`=VALUES(`title_snapshot`),`cover_url_snapshot`=VALUES(`cover_url_snapshot`),`spec_snapshot`=VALUES(`spec_snapshot`),`unit_price`=VALUES(`unit_price`),`quantity`=VALUES(`quantity`),`refund_quantity`=0;

INSERT INTO `qixi_crm_b_user_browse_history` (`id`,`user_id`,`product_id`,`store_id`,`viewed_at`) WITH RECURSIVE `screen_seq` AS (
  SELECT 1 AS `n` UNION ALL SELECT `n` + 1 FROM `screen_seq` WHERE `n` < 168
)
SELECT 977000 + `n`, 97200 + MOD(`n` - 1, 24) + 1, 1500 + MOD(`n` - 1, 8) + 1, 10 + MOD(`n` - 1, 8) + 1,
  DATE_SUB(NOW(), INTERVAL (`n` * 7) MINUTE)
FROM `screen_seq`
ON DUPLICATE KEY UPDATE `user_id`=VALUES(`user_id`),`product_id`=VALUES(`product_id`),`store_id`=VALUES(`store_id`),`viewed_at`=VALUES(`viewed_at`);
