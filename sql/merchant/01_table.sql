...
CREATE TABLE IF NOT EXISTS `qixi_crm_m_product` (
  ...
  `category_id` bigint unsigned DEFAULT NULL, `status` enum('draft','pending_review','on_sale','off_sale','rejected') NOT NULL DEFAULT 'draft',
...
CREATE TABLE IF NOT EXISTS `qixi_crm_m_product_tag` (
  ...
  `status` tinyint NOT NULL DEFAULT 1, PRIMARY KEY (`id`), UNIQUE KEY `uk_store_tag` (`store_id`,`name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS `qixi_crm_m_aftersale_action` (
  ...
  `attachments` json NOT NULL, `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`), KEY `idx_refund_time` (`refund_id`,`created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
