-- 平台「社区分类」API 按钮权限（幂等）
-- RequireAdminMenu 仅认 kind=button；page 码 content.community.category 不能用于接口鉴权。
...
INSERT INTO `qixi_crm_a_menu` (`id`,`parent_id`,`code`,`title`,`icon`,`route_path`,`kind`,`sort`,`status`) VALUES
  ...
  (21075,85,'content.community_category.read','查看社区分类','','community/category','button',1,1),
  (21076,85,'content.community_category.manage','维护社区分类','','community/category','button',2,1)
