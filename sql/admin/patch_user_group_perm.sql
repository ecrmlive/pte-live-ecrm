-- 平台「用户分组」API 按钮权限（幂等）
-- RequireAdminMenu 仅认 kind=button；page 码 user.group 不能用于接口鉴权。
...
INSERT INTO `qixi_crm_a_menu` (`id`,`parent_id`,`code`,`title`,`icon`,`route_path`,`kind`,`sort`,`status`) VALUES
  ...
  (21060,72,'user.group.read','查看用户分组','','user/group','button',1,1),
  (21061,72,'user.group.manage','维护用户分组','','user/group','button',2,1)
