...
func (s *Service) CreatePlatformAdmin(ctx context.Context, in AdminSaveInput) (*SystemAdmin, error) {
	...
	regionIDs := ""
	if in.RegionIDs != nil {
		regionIDs = normalizeIDCSV(*in.RegionIDs)
	}
	row := &SystemAdmin{
	...
	if in.IsAgent != nil {
		if *in.IsAgent != 0 && *in.IsAgent != 1 {
			return nil, ErrBadParam
		}
		row.IsAgent = *in.IsAgent
	}
	if row.IsAgent == 1 && row.RegionIDs == "" {
		return nil, ErrBadParam
	}
	if in.CircleAgentID != nil {
		row.CircleAgentID = *in.CircleAgentID
	}
...
func (s *Service) UpdatePlatformAdmin(ctx context.Context, id, actorID uint, in AdminSaveInput) (*SystemAdmin, error) {
	...
	if in.RegionIDs != nil {
		row.RegionIDs = normalizeIDCSV(*in.RegionIDs)
	}
	if row.IsAgent == 1 && row.RegionIDs == "" {
		return nil, ErrBadParam
	}
...
func normalizeIDCSV(raw string) string {
	ids := parseIDs(raw)
...
func (s *Service) CreateRole(ctx context.Context, merID uint, isAgent int8, in RoleSaveInput) (*SystemRole, error) {
	...
	if (merID == 0 && isAgent != 0 && isAgent != 1) || (merID != 0 && isAgent != 2) {
		return nil, ErrBadParam
	}
	circleID := uint(0)
	if in.CircleID != nil {
	...
	row := &SystemRole{
		...
		MerID: merID, IsAgent: isAgent, CircleID: circleID,
	}
