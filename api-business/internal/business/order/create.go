	...
	err = db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		for _, store := range checkout.Stores {
			...
			for _, line := range store.Lines {
				item := orderItemRow{OrderID: order.ID, ProductID: line.ProductID, SKUKey: line.SKUKey, TitleSnapshot: line.Title, CoverURLSnapshot: line.CoverURL, SpecSnapshot: "{}", UnitPrice: money(line.UnitCents), Quantity: line.Quantity}
