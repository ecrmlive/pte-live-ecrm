...
compose_in_dir() {
	...
	if [[ -n "${key}" && "${action}" != ps && "${action}" != config ]]; then
	...
	(
		...
		up)
			...
			;;
		down)
			docker compose -p "${project}" -f docker-compose.yaml --env-file "${env_file}" down
			;;
		restart)
...
compose_service() {
	...
	if ! service_has_compose "${key}"; then
		echo "错误: ${key} 无 Docker compose" >&2
		exit 1
	fi
...
SVC_ARGS='${compose_svc}'
if [[ -n "\${SVC_ARGS}" ]]; then
	docker compose -p '${project}' -f docker-compose.yaml --env-file '${env_file}' up -d \${SVC_ARGS}
elif [[ '${key}' == db ]]; then
	docker compose -p '${project}' -f docker-compose.yaml --env-file '${env_file}' up -d
...
compose_check_all() {
	local key
	for key in api-admin api-app job admin merchant-admin h5 pc service-web manager; do
		echo ">> compose config check ${key}"
		compose_service config "${key}"
	done
	echo ">> compose config OK（project: qixi_mergers=业务；IM=db/mq；对象存储=腾讯云 COS）"
}
