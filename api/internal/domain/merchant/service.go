...
type Store interface {
	ListMerchants(ctx context.Context, keyword string, status *int8, page, limit int) ([]Merchant, int64, error)
...
func (s *Service) ListMerchants(ctx context.Context, keyword string, status *int8, page, limit int) (*PageResult[Merchant], error) {
	list, total, err := s.store.ListMerchants(ctx, keyword, status, page, limit)
	if err != nil {
		return nil, err
	}
	if page <= 0 {
