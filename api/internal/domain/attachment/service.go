...
type Store interface {
	...
	List(ctx context.Context, userType int, cateID uint, page, limit int) ([]Attachment, int64, error)
...
func (s *Service) List(ctx context.Context, userType int, cateID uint, page, limit int) (*PageResult[Attachment], error) {
	if page < 1 {
		page = 1
	}
	...
	list, total, err := s.store.List(ctx, userType, cateID, page, limit)
	if err != nil {
		return nil, err
	}
...
func (s *Service) CreateFile(ctx context.Context, userType int, userID, cateID uint, name, src string) (*Attachment, error) {
	...
	a := &Attachment{
		...
		CreateTime: time.Now(), AttachmentType: 0,
	}
