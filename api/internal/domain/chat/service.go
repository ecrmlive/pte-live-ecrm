...
import (
	...
	"github.com/qixi-live/qixi-live-mergers/api/internal/infra/imclient"
	"gorm.io/gorm"
)
...
type IMSettings struct {
	...
	AppID       string
	Token       string
	Secret      string // 本地 UserSig HMAC
}

type Service struct {
	store Store
	im    IMSettings
	remote *imclient.Client
}
...
func NewService(store Store, im IMSettings) *Service {
	...
	var remote *imclient.Client
	if strings.EqualFold(im.Mode, "remote") && strings.TrimSpace(im.APIBase) != "" && strings.TrimSpace(im.Token) != "" {
		remote = imclient.New(im.APIBase, im.Token, appID)
	}
	return &Service{store: store, im: im, remote: remote}
}
...
func (s *Service) IssueCredentialForThread(ctx context.Context, portal string, localID uint, threadID uint) (*Credential, error) {
	...
	var convID uint64
	if threadID > 0 {
		if t, err := s.store.GetThread(ctx, threadID); err == nil {
			_ = s.ensureIMConversation(ctx, t)
...
func (s *Service) ensureIMConversation(ctx context.Context, t *Thread) error {
	if t == nil || s.remote == nil || !s.remote.Enabled() {
		return nil
	}
	...
	convID, err := s.remote.OpenSingle(ctx, userNum, svcNum)
	if err != nil {
		return err
	}
