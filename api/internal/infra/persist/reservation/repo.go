...
func (r *Repo) ListProducts(ctx context.Context, merID *uint, page, limit int) ([]reservation.ProductView, int64, error) {
	...
	q := r.db.WithContext(ctx).Table("qixi_store_product AS p").
	...
	err := q.Select(`p.product_id, p.mer_id, p.store_name, p.image, p.price, p.ot_price, p.stock,
		...
		Joins("LEFT JOIN qixi_merchant m ON m.mer_id = p.mer_id").
		Joins("LEFT JOIN qixi_store_product_reservation r ON r.product_id = p.product_id AND r.is_del = 0").
...
func (r *Repo) CountBooked(ctx context.Context, productID, slotID uint, date string) (int64, error) {
	var n int64
	err := r.db.WithContext(ctx).Table("qixi_store_order AS o").
		Joins("INNER JOIN qixi_store_order_product op ON op.order_id = o.order_id").
