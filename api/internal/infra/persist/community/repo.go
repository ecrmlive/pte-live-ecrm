...
func (r *Repo) LoadUserNickname(ctx context.Context, uid uint) (string, error) {
	var nick string
	err := r.db.WithContext(ctx).Table("qixi_user").Select("nickname").Where("uid = ?", uid).Scan(&nick).Error
	if err != nil {
		return "", err
	}
	if nick == "" {
...
func (r *Repo) LoadTopicName(ctx context.Context, id uint) (string, error) {
	...
	var name string
	err := r.db.WithContext(ctx).Table("qixi_community_topic").Select("topic_name").
...
func (r *Repo) LoadCateName(ctx context.Context, id uint) (string, error) {
	...
	var name string
	err := r.db.WithContext(ctx).Table("qixi_community_category").Select("cate_name").
