...
func (h *Handler) Login(c *gin.Context) {
	...
	pair, err := h.jwt.Issue(authjwt.PortalService, staff.ServiceID, staff.MerID, 0, staff.Account)
	if err != nil {
...
func (h *Handler) Refresh(c *gin.Context) {
	...
	claims, err := h.jwt.ParseExpect(req.RefreshToken, authjwt.PortalService, authjwt.TokenRefresh)
	if err != nil || claims.MerID == 0 || claims.AdminID == 0 {
		response.Fail(c, http.StatusUnauthorized, "刷新令牌无效")
		return
	}
	pair, err := h.jwt.Issue(authjwt.PortalService, claims.AdminID, claims.MerID, 0, claims.Account)
	if err != nil {
		response.Fail(c, http.StatusInternalServerError, "签发令牌失败")
		return
	}
