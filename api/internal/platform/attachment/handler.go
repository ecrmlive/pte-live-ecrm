...
func (h *Handler) Register(r gin.IRoutes) {
	...
	r.POST("/attachments/categories", h.CreateCategory)
	r.PUT("/attachments/categories/:id", h.UpdateCategory)
...
func (h *Handler) List(c *gin.Context) {
	...
	res, err := h.svc.List(c.Request.Context(), 0, uint(cateID), page, limit)
	if err != nil {
		response.Fail(c, http.StatusInternalServerError, "查询失败")
		return
	}
...
func (h *Handler) Upload(c *gin.Context) {
	...
	row, err := h.svc.CreateFile(c.Request.Context(), 0, middleware.AdminID(c), uint(cateID), name, src)
	if err != nil {
		writeErr(c, err)
		return
	}
