...
func (h *Handler) Login(c *gin.Context) {
	...
	admin, err := h.svc.LoginPlatform(c.Request.Context(), req.Account, req.Password, c.ClientIP())
	if err != nil {
		writeIdentityErr(c, err)
		return
	}
	pair, err := h.jwt.Issue(authjwt.PortalPlatform, admin.AdminID, 0, 0, admin.Account)
	if err != nil {
...
func (h *Handler) Refresh(c *gin.Context) {
	...
	claims, err := h.jwt.ParseExpect(req.RefreshToken, authjwt.PortalPlatform, authjwt.TokenRefresh)
	if err != nil {
		response.Fail(c, http.StatusUnauthorized, "刷新令牌无效")
		return
	}
	pair, err := h.jwt.Issue(authjwt.PortalPlatform, claims.AdminID, 0, 0, claims.Account)
	if err != nil {
		response.Fail(c, http.StatusInternalServerError, "签发令牌失败")
		return
	}
...
func (h *Handler) Me(c *gin.Context) {
	profile, err := h.svc.PlatformProfile(c.Request.Context(), middleware.AdminID(c))
	if err != nil {
		writeIdentityErr(c, err)
		return
	}
...
func (h *Handler) Menus(c *gin.Context) {
	...
	menus, err := h.svc.MenusForPlatform(c.Request.Context(), profile.Roles, profile.Level)
	if err != nil {
...
func (h *Handler) Permissions(c *gin.Context) {
	perms, err := h.svc.PlatformPermissionPaths(c.Request.Context(), middleware.AdminID(c))
	if err != nil {
...
func (h *Handler) ChangePassword(c *gin.Context) {
	...
	err := h.svc.ChangePlatformPassword(c.Request.Context(), middleware.AdminID(c), req.OldPassword, req.NewPassword)
	if err != nil {
		writeIdentityErr(c, err)
		return
	}
