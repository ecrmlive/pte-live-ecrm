...
const storeGroupRBACAcceptanceUserBase uint = 987680150

func TestStoreGroupHTTPRBACAcrossFiveRoles(t *testing.T) {
	...
	for index := range roles {
		userIDs = append(userIDs, storeGroupRBACAcceptanceUserBase+uint(index))
	}
	cleanup := func() {
	...
	for index, role := range roles {
		...
		if err := db.Table("qixi_crm_a_admin_user").Create(map[string]any{
			"id":                 userID,
			"username":           "store-group-rbac-" + role,
			"password_hash":      "not-used-by-rbac-test",
			"display_name":       "中文店铺分组权限验收-" + role,
			"status":             1,
			"auth_version":       1,
	...
	call := func(role string, userID uint) int {
		pair, issueErr := jwtManager.IssueAdminConsole(userID, "store-group-rbac-"+role, []string{role}, 1)
		if issueErr != nil {
			t.Fatal(issueErr)
		}
		req := httptest.NewRequest(http.MethodGet, "/api/platform/v1/store-groups", nil)
	...
	unauthenticated := httptest.NewRequest(http.MethodGet, "/api/platform/v1/store-groups", nil)
