// Package productmeta owns platform-wide product labels, guarantees,
// parameter templates and price rules. Store-specific metadata remains in api-merchant.
package productmeta
...
func NewHandler(db *gorm.DB, merchantDB ...*gorm.DB) *Handler {
	h := &Handler{db: db}
	if len(merchantDB) > 0 {
		h.merchantDB = merchantDB[0]
	}
	return h
}
...
func (h *Handler) Register(r gin.IRoutes) {
	...
	h.registerStoreParameters(r, platform)
	h.registerPriceRules(r, platform)
}
...
type guarantee struct {
	...
	Sort         int       `gorm:"column:sort" json:"sort"`
	Status       int8      `gorm:"column:status" json:"status"`
	MerCount     int       `gorm:"column:mer_count" json:"mer_count"`
...
func (h *Handler) registerLabels(r gin.IRoutes, platform gin.HandlerFunc) {
	...
	r.PUT("/product/labels/:id/status", platform, manage, h.updateLabelStatus)
...
func (h *Handler) registerGuarantees(r gin.IRoutes, platform gin.HandlerFunc) {
	...
	r.PUT("/product/guarantees/:id/status", platform, manage, h.updateGuaranteeStatus)
...
func (h *Handler) updateLabelStatus(c *gin.Context) {
	var in struct {
		Status int8 `json:"status"`
	}
	if id(c) == 0 || c.ShouldBindJSON(&in) != nil || !validStatus(in.Status) {
		bad(c, "商品标签状态参数错误")
		return
	}
	res := h.db.WithContext(c.Request.Context()).Model(&label{}).Where("id=?", id(c)).Update("status", in.Status)
	if res.Error != nil {
		internal(c, "更新商品标签状态失败")
		return
	}
	if res.RowsAffected != 1 {
		notFound(c, "商品标签不存在")
...
func (h *Handler) listGuarantees(c *gin.Context) {
	...
	q := h.db.WithContext(c.Request.Context()).Model(&guarantee{})
...
func (h *Handler) updateGuaranteeStatus(c *gin.Context) {
	guaranteeID := id(c)
	if guaranteeID == 0 {
		bad(c, "ID 错误")
		return
	...
	if c.ShouldBindJSON(&in) != nil || !validStatus(in.Status) {
		bad(c, "保障服务状态参数错误")
		return
	}
	res := h.db.WithContext(c.Request.Context()).Model(&guarantee{}).Where("id=?", guaranteeID).Update("status", in.Status)
	if res.Error != nil {
		internal(c, "更新保障服务状态失败")
		return
	}
	if res.RowsAffected != 1 {
		notFound(c, "保障服务不存在")
...
func validGuarantee(in guaranteeInput) bool {
	contentLen := len([]rune(strings.TrimSpace(in.Content)))
	return validName(in.Name, 64) && contentLen > 0 && contentLen <= 1000 && len([]rune(strings.TrimSpace(in.IconURL))) <= 1024 && validStatus(in.Status)
}
