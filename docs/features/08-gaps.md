# 功能点基线解析清单（不代表 Vben 实现已完成）

> 基于菜单按钮 + 路由补全后的**功能基线解析**，并已对照 `crmeb-system-menu.tsv` 分类。
>
> 本文件只回答“CRMEB 的功能范围是否已被识别”，**不回答七禧的 Vben 页面、接口、权限或交互是否已实现**。动态菜单叶子当前仍存在 `ecrm/placeholder/index.vue` 回退，不能据此文件或“真缺口 = 0”宣称后台已经对齐。实际开发验收统一见 [`../crmeb-vben-parity.md`](../crmeb-vben-parity.md)。
...
| 平台后台 | 店铺 / 区域代理 | `/business-zones/manage` | 目录壳 | 子菜单 3 个（区域列表/代理人员/代理设置） |
...
| 平台后台 | 店铺 / 区域代理 / 代理审核（侧栏隐藏） | `/business-zones/agent-review` | CRD | U | 1 | 侧栏不展示；路由与审核 API 保留 |
