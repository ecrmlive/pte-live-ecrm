...
export type PayType = "balance" | "wechat" | "alipay";
...
export function updateAddress(addressID: number, data: Partial<Address>) {
  return http.put<Address>(`/address/${addressID}`, data as Record<string, unknown>);
}

export function removeAddress(addressID: number) {
  return http.delete<{ ok: boolean }>(`/address/${addressID}`);
}
...
export function payOrder(groupOrderId: number, pay_type: PayType) {
  return http.post<GroupOrder | PayIntent>(`/order/pay/${groupOrderId}`, { pay_type });
}

export function fetchOrderPaymentChannels(groupOrderId: number) {
  return http.get<{ list: PaymentChannel[] }>(`/order/pay/${groupOrderId}/channels`);
}
