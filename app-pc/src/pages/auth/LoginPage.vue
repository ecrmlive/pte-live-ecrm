...
import { verifyWithPteCaptcha, type CaptchaAction } from "@/lib/pte-captcha";
...
function resetFeedback() { message.value = ""; captchaToken.value = ""; captchaAction.value = ""; }
function selectLoginMode(mode: "password" | "sms") { loginMode.value = mode; resetFeedback(); }
...
async function completeCaptcha(action: CaptchaAction) {
  ...
  try {
    ...
    message.value = (error as ApiError).message || "安全验证未完成";
  } finally {
...
async function submit() {
  ...
  if (page.value === "login" && loginMode.value === "password") {
    ...
    if (!captchaToken.value || captchaAction.value !== "login_password") { message.value = "请先完成图形验证码"; return; }
    loading.value = true;
    try {
  ...
  <div class="login-mask" role="dialog" aria-modal="true" aria-label="用户登录">
    <form class="login-modal" @submit.prevent="submit">
      ...
      <div v-if="page === 'login' && loginMode === 'password'" class="fields">
        ...
        <label class="code-field"><input :value="captchaAction === 'login_password' ? '图形验证已完成' : ''" readonly placeholder="请完成图形验证码" /><button type="button" class="captcha-action" :disabled="captchaBusy" @click="completeCaptcha('login_password')">{{ captchaBusy ? '验证中…' : captchaAction === 'login_password' ? '重新验证' : '图形验证' }}</button></label>
      </div>
