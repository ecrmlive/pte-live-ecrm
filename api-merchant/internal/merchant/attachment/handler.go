...
func (h *Handler) List(c *gin.Context) {
	...
	res, err := h.svc.List(c.Request.Context(), merID, uint(cateID), typeValue, page, limit)
	if err != nil {
		response.Fail(c, http.StatusInternalServerError, "查询失败")
		return
	}
...
func (h *Handler) Upload(c *gin.Context) {
	...
	row, err := h.svc.CreateFile(c.Request.Context(), int(merID), middleware.AdminID(c), uint(cateID), name, src, uploadType(fh.Filename, fh.Header.Get("Content-Type")))
	if err != nil {
		writeErr(c, err)
		return
	}
