...
func NewHandler(svc *presell.Service, id *identity.Service) *Handler {
	return &Handler{svc: svc, id: id}
}

func (h *Handler) Register(r gin.IRoutes) {
	r.GET("/presell/actives", h.List)
	r.POST("/presell/actives", middleware.RequireMerchantMenu(h.id, identity.MerPermPresellCreate), h.Create)
	r.PUT("/presell/actives/:id", h.Update)
	r.PUT("/presell/actives/:id/show", middleware.RequireMerchantMenu(h.id, identity.MerPermPresellToggle), h.SetShow)
	r.DELETE("/presell/actives/:id", middleware.RequireMerchantMenu(h.id, identity.MerPermPresellDelete), h.Delete)
}
...
func (h *Handler) Update(c *gin.Context) {
	...
	if in.IsShow != nil {
		if err := h.id.RequireMerchantMenu(c.Request.Context(), middleware.AdminID(c), identity.MerPermPresellToggle); err != nil {
...
func writeErr(c *gin.Context, err error) {
	...
	case errors.Is(err, presell.ErrBadParam), errors.Is(err, presell.ErrNotFullPay):
