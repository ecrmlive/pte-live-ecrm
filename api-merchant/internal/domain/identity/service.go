...
func (s *Service) PlatformProfile(ctx context.Context, adminID uint) (*PlatformProfile, error) {
	...
	return &PlatformProfile{
		...
		CircleAgentID: admin.CircleAgentID,
	}, nil
}

// PlatformRegionScope 返回平台账号可访问的商户区域：nil 表示平台全量，空切片表示区域账号尚未绑定区域。
func (s *Service) PlatformRegionScope(ctx context.Context, adminID uint) ([]uint, error) {
	...
	return parseIDs(profile.RegionIDs), nil
}
