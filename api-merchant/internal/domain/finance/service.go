...
type Store interface {
	...
	ListPlatformFinancialsByRegions(ctx context.Context, regionIDs []uint, status *int, page, limit int) ([]Financial, int64, error)
	GetPlatformFinancialByRegions(ctx context.Context, id uint, regionIDs []uint) (*Financial, error)
...
// ListPlatformByRegions 返回区域管理员可监管的提现单；nil 表示平台全量，空范围明确返回空列表。
func (s *Service) ListPlatformByRegions(ctx context.Context, regionIDs []uint, status *int, page, limit int) (*PageResult[Financial], error) {
	if regionIDs == nil {
		return s.ListPlatform(ctx, status, page, limit)
	}
	page, limit = normalizePage(page, limit)
	if len(regionIDs) == 0 {
		return &PageResult[Financial]{List: []Financial{}, Total: 0, Page: page, Limit: limit}, nil
	}
	list, total, err := s.store.ListPlatformFinancialsByRegions(ctx, regionIDs, status, page, limit)
	if err != nil {
		return nil, err
	}
	return &PageResult[Financial]{List: list, Total: total, Page: page, Limit: limit}, nil
...
// GetPlatformByRegions 返回区域管理员可监管的提现详情。查询必须在 SQL 层关联商户区域。
func (s *Service) GetPlatformByRegions(ctx context.Context, id uint, regionIDs []uint) (*Financial, error) {
	if regionIDs == nil {
	...
	f, err := s.store.GetPlatformFinancialByRegions(ctx, id, regionIDs)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrNotFound
		}
